...
module ActiveStorage
  class Service::AliyunService < Service
    ...
    def object_url(key, sign:, expires_in: 60, params: {})
      ...
      puts "String to sing #{string_to_sign}"
...
#   def token
#     return @token unless @token.nil? || token_expired?

#     policy = Aliyun::STS::Policy.new
#     policy.allow(['oss:*'], ["acs:oss:*:1557106377546687:#{config.fetch(:bucket)}/*"])
#     @token = sts.assume_role('acs:ram::1557106377546687:role/exequte-oss-full', 'client-01', policy)
#     @token
#   end
...
#       access_key_id:
#       config.fetch(:access_key_id),
#       access_key_secret:
#       config.fetch(:access_key_secret)
#     )
#   end
